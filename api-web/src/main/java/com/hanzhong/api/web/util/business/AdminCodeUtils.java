...
public class AdminCodeUtils {
    ...
    private static void cacheCodeMap() {
        try {
            codeMap = CommonUtils.cacheCodeMapExcludeChineseRow(ADMIN_CODE_FILE_PATH, ADMIN_CODE_FILE_SHEET_NAME);
        } catch (IOException e) {
            logger.error("文件：【{}】，sheet：【{}】,缓存代码(cacheCodeMap())出现异常：", ADMIN_CODE_FILE_PATH, ADMIN_CODE_FILE_SHEET_NAME, e);
        }
    }
