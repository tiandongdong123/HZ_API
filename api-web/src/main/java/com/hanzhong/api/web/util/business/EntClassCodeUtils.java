...
public class EntClassCodeUtils {
    ...
    private static void cacheCodeMap() {
        try {
            codeMap = CommonUtils.cacheCodeMapExcludeChineseRow(ENT_CLASS_CODE_FILE_PATH, ENT_CLASS_CODE_FILE_SHEET_NAME);
        } catch (IOException e) {
            logger.error( "文件：【{}】，sheet：【{}】,缓存代码(cacheCodeMap())出现异常：", ENT_CLASS_CODE_FILE_PATH, ENT_CLASS_CODE_FILE_SHEET_NAME, e);
        }
    }
