...
public class LocalAdminCodeUtils {
    ...
    private static void cacheCodeMap() {
        try {
            ...
            codeParam.setFilePath(LOCAL_ADMIN_CODE_FILE_PATH);
            codeParam.setSheetName(LOCAL_ADMIN_CODE_FILE_SHEET_NAME);
            codeParam.setCodeColumnNum(0);
            codeParam.setNameColumnNum(1);
            codeMap = CommonUtils.cacheCodeMapExcludeChineseRow(codeParam);
        } catch (IOException e) {
            LoggerUtils.appendErrorLog(logger, "文件：【{}】，sheet：【{}】,缓存代码(cacheCodeMap())出现异常：", LOCAL_ADMIN_CODE_FILE_PATH, LOCAL_ADMIN_CODE_FILE_SHEET_NAME, e);
        }
    }
